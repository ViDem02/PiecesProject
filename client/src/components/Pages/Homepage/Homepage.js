...
export const Homepage = () => {
	...
	return (
		<div className={"mt-3 w-100 d-flex flex-column justify-content-center align-items-center"}>
			...
			<div className={"h3 text-center w-100"}>
				Status Panel
			</div>
			<div className={"w-75 h-75 border mt-3 p-4"}>
				<div className="d-flex w-100 justify-content-center">
					<button className={"btn btn-primary"} onClick={refreshData}>Refresh Status</button>
				</div>
				{
					statusRes ? statusRes?.data?.map(s => <Status condition={s.status} text={s.device}/>)
						: <div className={"w-100 text-center"}>Fetching status data...</div>
				}
